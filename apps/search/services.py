...
async def get_similar_values(value: SearchRequest):
    ...
    length = len(models)
    if length == 0:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"no data found in {value.file_name}")
    ...
    if output.empty:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="no data found for ypur request")
    return {
